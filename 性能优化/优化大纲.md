从输入 url 到展示界面发生了什么，在这一完整过程中的各个步骤进行优化

1.dns 解析 2.请求 html 文件（cdn） 3.缓存 4.解析 html（css、dom、script） 5.资源压缩
图片压缩
js、css 压缩
缓存
字体拆分，字体是按需加载的，使用了的字体才会加载
...
prefetch 和 preload：（例子：preload-prefetch-master）
...
moment 改成 dayjs
lodash 改成 lodash-es
安装配置 webpack-bundle-analyzer，查看依赖有没有重复，不同版本可能会多次引用
