css和script的加载和解析都会阻塞html的解析和渲染，因为要先构建dom树和cssom树，然后二者结合生成渲染树，但是，外链的css和js都有可能加载失败，难道html就不解析了吗，显然不是，应该是有一定的时间限制，超出一定时间没加载成功css或者js，就跳过加载继续执行html的解析和渲染。这个预先解析的过程是浏览器的优化手段
